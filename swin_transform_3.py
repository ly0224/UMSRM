...
class LNet(nn.Module):
    def __init__(self, config):
        ...
        # Fully connected layers for predicting scaling factor
        ...
        self.fc = nn.Sequential(
            ...
            nn.Linear(64, 1, bias=False),  # Predicting a single scaling factor
        )
    ...
    def forward(self, x):
        ...
        # Predict the scaling factor
        scale_factor = self.fc(x)  # The predicted scaling factor (e.g., between 0.5 and 2.0)

        self.fc_output = self.fc(x)

        # Create scaling matrix for batch
        scaling_matrix = torch.eye(3, device=x.device).unsqueeze(0).repeat(B, 1, 1)
        scaling_matrix[:, 0, 0] = scale_factor.view(-1)
        scaling_matrix[:, 1, 1] = scale_factor.view(-1)

        # Remove translation from matrix
        # Translation part is removed since we don't want to shift the image
        # Keep the scaling matrix unchanged

        # Use the scaling matrix with grid_sample
        # Note: affine_grid expects the affine matrix to be of shape (B, 2, 3)
        scaling_matrix = scaling_matrix[:, :2, :]  # Drop the last row for grid_sample
        grid = F.affine_grid(scaling_matrix[:, :2, :], source.size(), align_corners=True)
        scaled_images = F.grid_sample(source, grid, align_corners=True, mode='bicubic')

        return scaled_images, scale_factor
