...
class SinPositionalEncoding2D(nn.Module):
    ...
    def forward(self, tensor):
        tensor = tensor.permute(0, 2, 3, 1)  # (batch_size, height, width, channels)
...
class RegistrationHead(nn.Sequential):
    def __init__(self, in_channels, kernel_size=3):
        # 卷积层，输出6个仿射变换参数
        conv2d = nn.Conv2d(in_channels, 6, kernel_size=kernel_size, padding=kernel_size // 2)
    ...
    def forward(self, x):
        x = super().forward(x)  # 进行卷积操作，输出形状为 [4, 6, 256, 256]

        # 全局平均池化，输出形状为 [4, 6, 1, 1]
        x = F.adaptive_avg_pool2d(x, (1, 1))

        # 移除多余维度，得到形状为 [4, 6]
        x = x.view(x.size(0), -1)

        # 转换为仿射矩阵 2x3，输出形状为 [4, 2, 3]
        theta = x.view(-1, 2, 3)

        return theta

def visualize_features(features, batch_index=0):
    """
    可视化特征图
    :param features: 特征图张量，形状为 [B, C, H, W]
    :param batch_index: 选择可视化的批次索引
    """
    num_channels = features.shape[1]  # 通道数
    ...
    axes = axes.flatten()  # 展平 axes 以简化索引

    for i in range(num_channels):
        ax = axes[i]

        # 选择特征图并显示
        feature_map = features[batch_index, i, :, :].detach().cpu().numpy()
...
class Discriminator(nn.Module):
    def __init__(self, input_channels=1):
        ...
        self.model = nn.Sequential(
            ...
            nn.Sigmoid(),
            nn.AdaptiveAvgPool2d(1)  # 添加全局平均池化层，将特征图归一化为 (B, 1, 1, 1)
        )

    def forward(self, x):
        x = self.model(x)
        return x.view(-1)  # 展平成一维向量，形状为 (B,)
...
class LNet(nn.Module):
    def __init__(self, config):
        ...
        self.positional_encoding = SinPositionalEncoding2D(config.embed_dim * 8)

        # Fully connected layer for predicting shear
        height, width = config.img_size
        final_feature_dim = embed_dim * 8

        self.fc_shear = nn.Sequential(
            nn.Linear(final_feature_dim * (height // 32) * (width // 32), 64, bias=False),
            nn.ReLU(inplace=True),
            nn.Linear(64, 2, bias=False),  # Predicting two shear factors: x and y
        )

    def forward(self, x):
        source = x[:, 1:2, :, :]  # Assuming source is a single-channel grayscale image
        B, _, H, W = source.shape

        # Get features from the Swin Transformer
        out_feats = self.transformer(x)

        # Use the last feature map directly
        x = out_feats[-1]
        ...
        # Predict shear (shear_x and shear_y)
        shear = self.fc_shear(x)  # Shape: (B, 2)
        shear_x, shear_y = shear[:, 0], shear[:, 1]  # Split into shear_x and shear_y
        shear_x = torch.tanh(shear_x) * 50  # Normalize shear_x to range (-50, 50)
        shear_y = torch.tanh(shear_y) * 50  # Normalize shear_y to range (-50, 50)

        # Convert shear to radians
        shear_x_rad = torch.deg2rad(shear_x)
        shear_y_rad = torch.deg2rad(shear_y)

        # Compute shear matrix components
        tan_shear_x = torch.tan(shear_x_rad)
        tan_shear_y = torch.tan(shear_y_rad)

        # Construct shear matrices (B, 3, 3)
        shear_matrices = torch.stack([
            torch.stack([torch.ones_like(tan_shear_x), tan_shear_x, torch.zeros_like(tan_shear_x)], dim=1),
            torch.stack([tan_shear_y, torch.ones_like(tan_shear_y), torch.zeros_like(tan_shear_y)], dim=1),
            torch.stack([torch.zeros_like(tan_shear_x), torch.zeros_like(tan_shear_x), torch.ones_like(tan_shear_x)],
                        dim=1)
        ], dim=1)  # Shape: (B, 3, 3)

        # Extract the first two rows for affine transformation
        affine_matrices = shear_matrices[:, :2, :]  # Shape: (B, 2, 3)

        # Apply the affine transformation to the source image
        grid = F.affine_grid(affine_matrices, source.size(), align_corners=False)
        transformed_images = F.grid_sample(source, grid, align_corners=False, mode='bicubic')

        return transformed_images, shear_x, shear_y
