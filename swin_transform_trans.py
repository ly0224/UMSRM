...
class LNet(nn.Module):
    def __init__(self, config):
        ...
        self.fc_translate = nn.Sequential(
    ...
    def forward(self, x):
        ...
        # Predict translation (translate_x and translate_y)
        translation = self.fc_translate(x)  # Shape: (B, 2)
        translate_x, translate_y = translation[:, 0], translation[:, 1]  # Split into translate_x and translate_y

        # Normalize translate_x and translate_y to a reasonable range, e.g., (-H/2, H/2) and (-W/2, W/2)
        translate_x = torch.tanh(translate_x)
        translate_y = torch.tanh(translate_y)

        # Construct affine matrices for translation (B, 2, 3)
        affine_matrices = torch.stack([
            torch.stack([torch.ones_like(translate_x), torch.zeros_like(translate_x), translate_x], dim=1),
            torch.stack([torch.zeros_like(translate_y), torch.ones_like(translate_y), translate_y], dim=1)
        ], dim=1)  # Shape: (B, 2, 3)
        ...
        return transformed_images, translate_x, translate_y
