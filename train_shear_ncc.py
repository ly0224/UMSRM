...
import numpy as np
from swin_transform_4 import CONFIGS as CONFIGS_TM
import swin_transform_4 as LNet
...
def calculate_theta_BA(shear_x, shear_y):
    """
    Calculate the transformation matrix theta_BA based on the given shear values.

    shear_x: Tensor of shape (batch_size, 1) or (batch_size,) representing shear along the x-axis in degrees
    shear_y: Tensor of shape (batch_size, 1) or (batch_size,) representing shear along the y-axis in degrees
    """
    # Remove the second dimension if it exists
    if shear_x.ndimension() == 2:
        shear_x = shear_x.squeeze(dim=1)
    if shear_y.ndimension() == 2:
        shear_y = shear_y.squeeze(dim=1)

    # Convert shear values from degrees to radians
    shear_x_rad = shear_x * (torch.pi / 180.0)
    shear_y_rad = shear_y * (torch.pi / 180.0)

    batch_size = shear_x.size(0)
    theta_BA = torch.zeros(batch_size, 2, 3, device=shear_x.device)

    # Set up the affine transformation matrix for shear
    theta_BA[:, 0, 0] = 1  # No scaling along x-axis
    theta_BA[:, 1, 1] = 1  # No scaling along y-axis
    theta_BA[:, 0, 1] = torch.tan(shear_x_rad)  # Shear along x-axis
    theta_BA[:, 1, 0] = torch.tan(shear_y_rad)  # Shear along y-axis
    theta_BA[:, 0, 2] = 0  # No translation in x
    theta_BA[:, 1, 2] = 0  # No translation in y
...
class ImageTransformPairDataset(Dataset):
    def __init__(self, img1_dir, img2_dir, restored_dir, transform_img1=None, transform_img2=None,
        ...
        self.img1_paths = [os.path.join(img1_dir, f"{i}_restored_scale_rot_shear.jpg") for i in range(1, 9001)]
        self.img2_paths = [os.path.join(img2_dir, f"{i}_restored_scale_rot.jpg") for i in range(1, 9001)]
        self.restored_paths = [os.path.join(restored_dir, f"{i}_restored_scale_rot_shear.jpg") for i in range(1, 9001)]
...
def train_model(img1_dir, img2_dir, restored_dir, config_key, device, num_epochs=50, batch_size=4, lr=1e-4,
    ...
    mean_img2 = [0.1875]
    std_img2 = [0.2013]
    mean_restored = [0.1852]
    std_restored = [0.2012]

    # 创建保存结果的目录
    results_dir = 'E:/download/megadepth_test_1500/C2FViT/training_results/shear_full/ncc'
    ...
    # Loss function
    criterion_shear = ImageNCCLoss()  # 使用MSE损失
    ...
    save_dir = 'E:/download/megadepth_test_1500/C2FViT/checkpoints/shear/aviation/ncc'
    os.makedirs(save_dir, exist_ok=True)

    # Training loop
    tensor_folder_path = 'E:/download/megadepth_test_1500/C2FViT/NCC/train/tensor_shear'
    ...
    for epoch in range(num_epochs):
        ...
        for batch_index, (img_pair, restored_img) in enumerate(
            ...
            _, shear_x, shear_y = generator(img_pair)

            # Print the predicted shear values every 50 batches
            if (batch_index + 1) % 50 == 0:
                print(
                    f"Epoch [{epoch + 1}/{num_epochs}], Batch [{batch_index + 1}/{len(train_loader)}], "
                    f"Predicted Shear X: {shear_x.detach().cpu().numpy()}, Predicted Shear Y: {shear_y.detach().cpu().numpy()}"
                )

            theta_BA_pred = calculate_theta_BA(shear_x, shear_y)

            img2 = img_pair[:, 1].unsqueeze(1)  # Extract second image from pair

            if theta_BA_pred.size() == theta_BA_gt_batch.size():
                loss_G, transformed_pred, transformed_GT = criterion_shear(theta_BA_pred, theta_BA_gt_batch, img2)
                loss_G.backward()
                optimizer_G.step()
        ...
        with torch.no_grad():
            for batch_index, (img_pair, restored_img) in enumerate(tqdm(eval_loader, desc="Evaluating")):
                ...
                if len(theta_BA_gt_list) == img_pair.size(0):
                    ...
                    # Print the predicted shears every 30 batches
                    ...
                    if theta_BA_pred.size() == theta_BA_gt_batch.size():
                        ...
                        loss_G, transformed_pred, transformed_GT = criterion_shear(theta_BA_pred, theta_BA_gt_batch,
                                                                                   img2)
...
if __name__ == '__main__':
    ...
    img1_dir = 'E:/download/megadepth_test_1500/C2FViT/affine/train/imgs1'
    img2_dir = 'E:/download/megadepth_test_1500/C2FViT/affine/train/imgs_restored_scale_rot'
    restored_dir = 'E:/download/megadepth_test_1500/C2FViT/affine/train_full/imgs_restored_scale_rot_shear'  # Path to the directory containing restored images
    ...
    model_path = 'E:/download/megadepth_test_1500/C2FViT/checkpoints/shear/aviation/LNet_best_TransMorph_Image_MSE_0.1882.pth'
