...
import numpy as np
from swin_transform import CONFIGS as CONFIGS_TM
import swin_transform as LNet
import pandas as pd
import math
...
def calculate_theta_BA(rotation_angle, degrees=True):
    """
    Calculate the transformation matrix theta_BA based on the predicted rotation angle.
    rotation_angle: Tensor of shape (batch_size, 1) or (batch_size,)
    degrees: Boolean, indicating if the input angles are in degrees
    """
    if rotation_angle.ndimension() == 2:
        rotation_angle = rotation_angle.squeeze(dim=1)  # Remove second dimension

    if degrees:
        rotation_angle = torch.deg2rad(rotation_angle)  # Convert to radians if necessary

    batch_size = rotation_angle.size(0)
    theta_BA = torch.zeros(batch_size, 2, 3, device=rotation_angle.device)

    theta_BA[:, 0, 0] = torch.cos(rotation_angle)
    theta_BA[:, 0, 1] = -torch.sin(rotation_angle)
    theta_BA[:, 1, 0] = torch.sin(rotation_angle)
    theta_BA[:, 1, 1] = torch.cos(rotation_angle)
    theta_BA[:, 0, 2] = 0  # Translation components if needed
    theta_BA[:, 1, 2] = 0
...
class ImageTransformPairDataset(Dataset):
    def __init__(self, img1_dir, img2_dir, restored_dir, transform_img1=None, transform_img2=None,
        ...
        self.img1_paths = [os.path.join(img1_dir, f"{i}.jpg") for i in range(1, 9001)]
        self.img2_paths = [os.path.join(img2_dir, f"{i}_restored_scale.jpg") for i in range(1, 9001)]
        self.restored_paths = [os.path.join(restored_dir, f"{i}_restored_scale_rot.jpg") for i in range(1, 9001)]
...
def train_model(img1_dir, img2_dir, restored_dir, config_key, device, num_epochs=50, batch_size=4, lr=1e-4,
    ...
    # 创建保存结果的目录
    results_dir = 'E:/download/megadepth_test_1500/C2FViT/training_results/rot/ncc'
    ...
    # Loss function
    criterion_angle = ImageNCCLoss()  # Using image-based MSE loss
    ...
    save_dir = 'E:/download/megadepth_test_1500/C2FViT/checkpoints/affine/rot/aviation/ncc'
    os.makedirs(save_dir, exist_ok=True)

    # Training loop
    tensor_folder_path = 'E:/download/megadepth_test_1500/C2FViT/affine/train/tensor_rotation'
    ...
    for epoch in range(num_epochs):
        ...
        for batch_index, (img_pair, restored_img) in enumerate(
            ...
            _, angle = generator(img_pair)

            # Print the predicted angles every 30 batches
            if (batch_index + 1) % 50 == 0:
                print(
                    f"Epoch [{epoch + 1}/{num_epochs}], Batch [{batch_index + 1}/{len(train_loader)}], Predicted Angles: {angle.detach().cpu().numpy()}")

            theta_BA_pred =calculate_theta_BA(angle, degrees=True)

            img2 = img_pair[:, 1].unsqueeze(1)  # Extract second image from pair

            if theta_BA_pred.size() == theta_BA_gt_batch.size():
                loss_G, transformed_pred, transformed_GT = criterion_angle(theta_BA_pred, theta_BA_gt_batch, img2)
                loss_G.backward()
                optimizer_G.step()
...
if __name__ == '__main__':
    ...
    img2_dir = 'E:/download/megadepth_test_1500/C2FViT/affine/train/imgs_restored_scale'
    restored_dir = 'E:/download/megadepth_test_1500/C2FViT/affine/train/imgs_restored_scale_rot'
    ...
    model_path = 'E:/download/megadepth_test_1500/C2FViT/checkpoints/affine/rot/aviation/ncc/LNet_best_TransMorph_Image_MSE_0.4150.pth'
